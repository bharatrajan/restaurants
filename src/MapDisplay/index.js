...
class MapDisplay extends Component {
  constructor(props) {
      super(props);
      this.directionsServiceResponseHandler = this.directionsServiceResponseHandler.bind(this);
  }
  ...
  calculateAndDisplayRoute = (nextProps) => {
    ...
    if(restaurentlatitude === "NOT_AVAILABLE" || 
        restaurentlongitude === "NOT_AVAILABLE" ) return;
    ...
    this.directionsService.route(requestBlob, this.directionsServiceResponseHandler);
  }
  ...
  resizeMap = () => {
    this.mapEl.style.height = this.mapEl.style.width = this.wrapper.offsetWidth + "px";      
  }

  render() {
      return(
        <div className="MapDisplay" ref={wrapper => this.wrapper = wrapper}>
            <div hidden className="align-text-left error-text" ref={errorEl => this.errorEl = errorEl}> Direction service failed </div>
